'''
    ...
    通过MRC任务加入任务语句与原语句组成两句任务，任务语句将任务指定为标注{O,T} 或 {O,POS,NEU,NEG}
    任务语句为“find the term"和"tag the sentiment"
'''
...
parser.add_argument('--check', type=str, default='model/absa_mrc_bertcrf',
...
class BERT(Layer):
    def __init__(self, **kwargs):
        ...
        self.mrc1 = self.tokenizer.encode("find the term", add_special_tokens=True, return_tensors="tf")
        self.mrc2 = self.tokenizer.encode("tag the sentiment", add_special_tokens=True, return_tensors="tf")

    def call(self, inputs, **kwargs):
        batch_size = tf.shape(inputs)[0]

        mrc1 = tf.tile(self.mrc1, [batch_size, 1])
        mrc2 = tf.tile(self.mrc2, [batch_size, 1])
        input1 = tf.concat([mrc1, inputs[:, 1:]], axis=1)
        input2 = tf.concat([mrc2, inputs[:, 1:]], axis=1)

        inputs_r = tf.concat([input1, input2], axis=0)

        token_type_ids = tf.tile(tf.concat([tf.zeros_like(mrc1), tf.ones_like(inputs[:, 1:])], axis=1), [2, 1])

        return self.bert(input_ids=inputs_r,
                         token_type_ids=token_type_ids,
                         attention_mask=tf.cast(tf.greater(inputs_r, 0), tf.int32)
                         )[0]
...
class USER:
    ...
    def build_model(self):
        ...
        predict = CRFMRC(label_dim=params.label_dim, name="crfmrc")(inputs=(sequence_split, lab, seqlen))
